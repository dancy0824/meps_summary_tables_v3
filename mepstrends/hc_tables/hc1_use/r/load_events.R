rename_sops = function(df,evnt,year,hosp=F){
  ...
  sops = c("SF","MR","MD","PV","VA","TR","OF","SL","WC","OT","OR","OU","XP")
  ...
  df %>% mutate(event=evnt, PR.yy.X=PV.yy.X+TR.yy.X, OZ.yy.X=OF.yy.X+SL.yy.X+OT.yy.X+OR.yy.X+OU.yy.X+WC.yy.X+VA.yy.X) %>%
         select(one_of("DUPERSID","event","SEEDOC","XP.yy.X","SF.yy.X","MR.yy.X","MD.yy.X","PR.yy.X","OZ.yy.X"))
}
...
OBV <- OBV %>% mutate(event_v2X=recode_factor(SEEDOC,.default='Missing','1'='OBD','2'='OBO'))
OPT <- OPT %>% mutate(event_v2X=recode_factor(SEEDOC,.default='Missing','1'='OPY','2'='OPZ'))

 ...
 * Program Name:    \\\programs.ahrq.local\programs\MEPS\AHRQ4_CY2\F_DSO\FF004ETH\Prog\SAS\totexp_ev.sas                                                                                                
 * Description :    SAS program to calculate one estimate (TOTEXP) by event type for 1996-2014                                                                                                                                                                                                  
...
RUN;
 ...
 /*
 format     sum_bil: comma20.0  stddev_bil: comma16.0;
 run;*/
...
%else %do;
 ...
 %demog(exp=dvexp, fmt1=DVT);
 %demog(exp=rxexp, fmt1=RX);
 %demog(exp=obexp, fmt1=OBV);
 %demog(exp=obyexp, fmt1=OBD);
 %demog(exp=obzexp, fmt1=OBO);
 %demog(exp=opexp, fmt1=OPT);
 %demog(exp=opyexp, fmt1=OPY);
 %demog(exp=opzexp, fmt1=OPZ);
 %demog(exp=erexp, fmt1=ERT);
 %demog(exp=ipexp, fmt1=IPT);
 %demog(exp=hhexp, fmt1=HHT);
 %demog(exp=hhaexp, fmt1=HHA);
 %demog(exp=hhnexp, fmt1=HHN);
 %demog(exp=otexp, fmt1=OMA);
 ...
 run;
  ...
  TITLE3 "total expenditures and SE for all event types, for 1996-2014";
...
data fyc9614;
set est.fyc9614(where=(grp1="ind" and grp2="event"));
...
outfile="\\programs.ahrq.local\programs\MEPS\AHRQ4_CY2\F_DSO\FF004ETH\Prog\SAS\Darya\QC\totev_unmatch.xls"
dbms= excel replace;
run;
...
outfile="\\programs.ahrq.local\programs\MEPS\AHRQ4_CY2\F_DSO\FF004ETH\Prog\SAS\Darya\QC\check5.xls"
dbms= excel replace;
run;

 ...
 * Description :    Calculate cross section estimates - # of events by demo*sop and compare them with V4 of R estimates for 1996-2014                                                                                                                                                                                                
...
PROC SORT DATA=TASK.EVNTDATA9614 (KEEP=YEAR DUPERSID SOP1-SOP5) OUT=EVNT;
  BY YEAR DUPERSID;
RUN;

DATA PUF;
  MERGE EVNT (IN=A) 
        PERS (IN=B KEEP=YEAR DUPERSID VARSTR  VARPSU PERWTF SEX AGEGRPS  AGEGRPS_V2  REGION    MARRIED  
...
PROC FREQ DATA=PUF;
TABLES    SEX AGEGRPS  AGEGRPS_V2  REGION    MARRIED  
          RACE INSURANCE   INSURANCE_V2  HEALTH  MENTAL_HEALTH EDUCATION   EMPLOYED     POVERTY /LIST MISSING;
RUN;
...
DATA FY&YEAR;
  ...
  TOTAL=1;
  XP=0;
  IF SOP1=1 OR SOP2=1 OR SOP3=1 OR SOP4=1 OR SOP5=1 THEN XP=1;
  IND=1;
  LEVELS1=&GRP1;
RUN;

ODS LISTING CLOSE;
PROC SURVEYMEANS DATA=FY&YEAR SUM STD sumwgt missing;
VAR TOTAL;
CLUSTER VARPSU; 
STRATA VARSTR;
WEIGHT PERWTF;
DOMAIN LEVELS1*XP*SUB LEVELS1*SOP1*SUB LEVELS1*SOP2*SUB LEVELS1*SOP3*SUB LEVELS1*SOP4*SUB LEVELS1*SOP5*SUB;
...
DATA OUT&N (DROP=LEVELS1);
  ...
  YEAR=&YEAR;
   ...
   L1=PUT(LEVELS1, &f1..);
   SAS_EST=ROUND(SUM/1000000, .1);
   SAS_SE=ROUND(STDDEV/1000000, .1);
   IF XP=1 THEN LEVELS2='XP';
   IF SOP1=1 THEN LEVELS2='SF';
   IF SOP2=1 THEN LEVELS2='PR';
   IF SOP3=1 THEN LEVELS2='MR'; 
   IF SOP4=1 THEN LEVELS2='MD';
   IF SOP5=1 THEN LEVELS2='OZ';
...
DATA ALL (RENAME=(L1=LEVELS1));
  SET OUT6 OUT1 OUT2 OUT3 OUT4 OUT5 OUT7 OUT8 OUT9 OUT10 OUT11 OUT12 OUT13 OUT14;
RUN;
...
DATA R;
SET V4R.EVT9614 (WHERE=(YEAR=&YEAR AND TOTEVT>0));
  GRP1=UPCASE(COMPRESS(GRP1));
  LEVELS1=UPCASE(COMPRESS(LEVELS1));
  GRP2=UPCASE(COMPRESS(GRP2));
  LEVELS2=UPCASE(COMPRESS(LEVELS2));
  
  R_EST=ROUND(TOTEVT/1000000, .1);
  R_SE=ROUND(TOTEVT_SE/1000000, .1);  

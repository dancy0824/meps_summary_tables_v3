...
Program Name: AHRQ3\F_DSO\FD001NKK\PROG\TOP10TC07_EXP.SAS

Description: Produces estimates with standard errors of the top 10 therapeutic classes (TC)  
             of drugs in 2007 by weighted expenditures for the Tables Compendia on the Web.
             Produce Z-scores comparing weighted expenditures of top 10 TC to each other.
...
Input file:   AHRQ3\F_DSO\FD001NKK\DATA\RX07V05.SAS7BDAT
...
OPTIONS FMTSEARCH=(FMT.FORMATS FMT.TC&YY._EXPFMT);
...
*--------------------------------------------------------------------------------------------
* Determine RANK for non-missing TC1 by weighted Rx expenditures.
*--------------------------------------------------------------------------------------------;     
DATA HASTC;
  SET PMEDS;
  IF TC1 NOT IN (-1, -9);
...
PROC SUMMARY DATA=HASTC NWAY;
  ...
  VAR RXXP&YY.X;
  WEIGHT PERWT&YY.F;
  OUTPUT OUT=TCFREQ (DROP=_TYPE_) SUM=RXEXPSUM;
RUN;

PROC SORT DATA=TCFREQ; BY DESCENDING RXEXPSUM; RUN;

PROC PRINT DATA=TCFREQ;
  SUM _FREQ_ RXEXPSUM;
  FORMAT _FREQ_ COMMA8.;
  FORMAT RXEXPSUM COMMA20.2;
  TITLE3 "WORK.TCFREQ: Sum of Rx expenditures for non-missing TC1, sorted in descending order";
  FOOTNOTE1 "Source file: AHRQ3\F_DSO\FD001NKK\DATA\&RXPUF..SAS7BDAT";
  FOOTNOTE2 "PERWT&YY.F > 0";
RUN;
...
DATA TCRANK;
  ...
  LABEL RXEXPSUM = "Weighted*Expenditures"
        RANK     = "TC rank by expenditures";        
...
PROC PRINT DATA=TCRANK;
  ...
  FORMAT _FREQ_ COMMA8.;
  FORMAT RXEXPSUM COMMA20.2;
  TITLE3 "WORK.TCRANK: Sum of Rx expenditures for non-missing TC1, sorted in descending order";
  FOOTNOTE1 "Source file: AHRQ3\F_DSO\FD001NKK\DATA\&RXPUF..SAS7BDAT";
  FOOTNOTE2 "PERWT&YY.F > 0";
RUN;
...
PROC FORMAT LIBRARY=FMT.TC&YY._EXPFMT CNTLIN=FMTTC FMTLIB; 
  TITLE3 "Contents of permanent format library for RANK";
  FOOTNOTE1 "C:\AHRQ3\F_DSO\FD001NKK\DATA\FMTLIB\TC&YY._EXPFMT.SAS7BCAT";
RUN;
...
PROC PRINT DATA=TCTAB.TOP10TC&YY._EXP NOOBS SPLIT="*";
  ID RANK;
  VAR NSUM
      TOT_USE SE_USE
      TOT_EXP SE_EXP
...
* Compute z-scores.
*---------------------------------------------------------------------------------------------;
PROC SORT DATA=TCTAB.TOP10TC&YY._EXP (KEEP=RANK TOT_EXP SE_EXP WHERE=(1 LE RANK LE 11))
                 OUT=SETABLE;
  BY RANK;
RUN;
...
PROC PRINT DATA=ZSCORE NOOBS SPLIT="*";
  ...
  LABEL TC1    = "RANK/TC #1"
        ...
        ZSTAT    = "Z-SCORE"
        PVALUE   = "P-VALUE";
  TITLE3 "Z Scores to Compare Top 10 Therapeutic Classes when Ranked by Weighted Expenditures";
RUN;

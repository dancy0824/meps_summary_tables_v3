...
meps_svyby <- list(
  'usc' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(FYCdsgn, ACCELI42==1 & HAVEUS42 >= 0 & LOCATN42 >= -1))',
  ...
  'diab' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=DIABdsgn)',
  
  'adult_nosmok'  = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(SAQdsgn, ADSMOK42==1 & CHECK53==1))',
  'adult_routine' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(SAQdsgn, ADRTCR42==1 & AGELAST >= 18))',
  'adult_illness' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(SAQdsgn, ADILCR42==1 & AGELAST >= 18))',
  
  'child_dental' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(FYCdsgn, child_2to17==1))',
  'child_routine' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(FYCdsgn, CHRTCR42==1 & AGELAST < 18))',
  'child_illness' = 'svyby(~.formula., FUN = .FUN., by = ~.by., design=subset(FYCdsgn, CHILCR42==1 & AGELAST < 18))',
 
  'adult' = 'svyby(~.formula., FUN=.FUN., by = ~.by., design=subset(SAQdsgn, ADAPPT42 >= 1 & AGELAST >= 18))',
  'child' = 'svyby(~.formula., FUN=.FUN., by = ~.by., design=subset(FYCdsgn, CHAPPT42 >= 1 & AGELAST < 18))'
)
